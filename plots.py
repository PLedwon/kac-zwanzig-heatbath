...
def theoDiff(x,a,b,c):
    return a*np.power(x,1.14)#+c

def linDiff(x,a,c):
    return a*x+c


startindex = int(math.floor((t1/dt)*0.60))
plotindex = int(math.floor((t1/dt)*0.60))
...
plt.plot(timesteps[startindex:endindex:8000],theoDiff(timesteps[startindex:endindex:8000],popt[0],popt[1],popt[2]),label=r'$\propto t^{1.93}$',color='#0066FF', linestyle='--')
